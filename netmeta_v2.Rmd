---
title: "Network Meta-Analysis, Version 2"
output:
  html_notebook:
    number_sections: yes
    toc: TRUE
...
foo3 <- function(ep, netplot = TRUE) {
  ...
  tmp3[, OR := (Events.x/N.x)/(Events.y/N.y)]
  tmp3[, logOR := log(OR)]
  tmp3[, seLogOR := sqrt((1/Events.x) + 
                           ...
                           (1/N.y))]
  tmp3$Compare <- "DAPT"
  tmp3 <- tmp3[, c(1:2, 10, 3:9)]
  print(datatable(tmp3,
                  caption = "",
                  rownames = FALSE,
  ...
  t3 <- data.table(Endpoint = ep,
                   Study = tmp3$Study,
                   Treatment1 = tmp3$Arm,
                   Treatment2 = tmp3$Compare,
                   N = tmp3$N.x + tmp3$N.y,
                   OR = tmp3$OR,
                   LB = exp(tmp3$logOR - 1.96*tmp3$seLogOR),
                   UB = exp(tmp3$logOR + 1.96*tmp3$seLogOR))
...
foo4 <- function(ep, netplot = TRUE) {
  ...
  tmp <- t1[(Treatment1 %in% c("ASA only 6m",
                               "ASA only",
                               "P2Y12 only") & 
               Treatment2 == "DAPT"), ]
  t3 <- rbindlist(list(t3,
...
# Load data
```{r data}
dt1 <- fread("data/Short DAPT Excel Sheet 09302019.csv")

# Remove PCI Cure
dt1 <- dt1[Study != "PCI Cure", ]
...
## 3 Groups
```{r mb_3}
foo3("Maojor Bleeding")
```

## 4 Groups
```{r mb_4}
foo4("Maojor Bleeding")
```
...
## 3 Groups
```{r pep_3}
foo3("Primary end point")
```

## 4 Groups
```{r pep_4}
foo4("Primary end point")
```

# Composite of end point 
## 3 Groups
```{r cep_3}
foo3("Composite of end point")
```

## 4 Groups
```{r cep_4}
foo4("Composite of end point")
```
